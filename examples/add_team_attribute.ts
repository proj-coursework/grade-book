...
function normalizeEmail(email: string) {
  return email
    .trim()
    .toLowerCase()
    .replace(/@(jhu|jh|jhmi)\.edu$/, "@jh.edu");
}
...
const teamsByEmail = Object.fromEntries(
  teamsRows.map((row) => [normalizeEmail(row.email), row]),
);
...
const outCsv = csvStringify.stringify(updatedRows, {
  header: true,
  columns: header,
});
...
// Error reporting
const emailsInTarget = new Set(
  targetRows.map((row) => normalizeEmail(row["Email"] || "")),
);
const emailsInTeams = new Set(
  teamsRows.map((row) => normalizeEmail(row.email)),
);
const missingFromTarget = teamsRows.filter(
  (row) => !emailsInTarget.has(normalizeEmail(row.email)),
);
const missingFromTeams = targetRows.filter(
  (row) => !emailsInTeams.has(normalizeEmail(row["Email"] || "")),
);
const errors = {
